...
import { toDB, toPhase, toVSWR } from './parser';
...
export function render(el, entries, view, markers) {
    if (view === 'smith') {
    ...
    const yTitle = view === 'db' ? 'Magnitude (dB)' : view === 'phase' ? 'Phase (°)' : 'VSWR';
    const shapes = markers.map((f) => ({
        type: 'line',
        x0: f / 1e6,
        x1: f / 1e6,
        y0: 0,
    ...
    return Plotly.react(el, traces, {
        ...BASE_LAYOUT,
        title: plotTitle(entries, view),
        xaxis: { ...AXIS_STYLE, title: { text: 'Frequency (MHz)' } },
        yaxis: { ...AXIS_STYLE, title: { text: yTitle } },
        shapes,
    }, { responsive: true }).then(() => Plotly.Plots.resize(el));
}
function renderSmith(el, entries, markers) {
    const traces = [...smithGrid()];
    for (const { label, color, data } of entries) {
...
function plotTitle(entries, view) {
    ...
    if (view === 'smith') {
        ...
        const viewLabel = view === 'db' ? 'Magnitude (dB)' : view === 'phase' ? 'Phase' : 'VSWR';
        if (entries.length > 1) {
            params = `S11${entries.some((e) => e.data.ports === 2) ? ', S21' : ''} · ${viewLabel}`;
        }
        else {
            const { ports } = entries[0].data;
            const measured = ports === 1 ? 'S11'
                : view === 'vswr' ? 'S11, S22'
                    : 'S11–S22';
