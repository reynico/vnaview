...
function load(file) {
    ...
    reader.onload = (e) => {
        const data = parse(e.target.result, file.name);
...
function markerValue(freqHz) {
    if (compareMode)
        return '';
    const f = files.find((f) => f.name === activeFile);
    if (!f)
        return '';
    const pt = f.data.points.reduce((a, b) => Math.abs(b.freq - freqHz) < Math.abs(a.freq - freqHz) ? b : a);
    if (view === 'db')
        return `${toDB(pt.params[0]).toFixed(2)} dB`;
    if (view === 'phase')
        return `${toPhase(pt.params[0]).toFixed(1)}°`;
    if (view === 'vswr')
        return `VSWR ${toVSWR(pt.params[0]).toFixed(2)}`;
    return '';
}
...
function attachClickListener() {
    ...
    chartEl.on('plotly_click', (ev) => {
        const pt = ev.points?.find((p) => ['S11', 'S21', 'S12', 'S22'].some((n) => p.data.name.includes(n)));
        if (!pt)
            return;
        let freqHz;
        if (view === 'smith') {
            ...
            if (!ref)
                return;
            const closest = ref.data.points.reduce((a, b) => (b.params[0].re - pt.x) ** 2 + (b.params[0].im - pt.y) ** 2 <
